...
def atan2(y, x):
    # torch.atan2(0, 0) == 0, but the legacy ONNX exporter's symbolic decomposes
    # atan2 into y/x (see torch/onnx/symbolic_opset9.py), which is 0/0 = NaN at
    # the origin. Zero-padded particle slots hit exactly this point, and the NaN
    # then poisons the whole attention softmax downstream. Nudge the degenerate
    # (0, 0) point off the origin so both backends agree.
    both_zero = (x == 0) & (y == 0)
    x = torch.where(both_zero, torch.ones_like(x), x)
...
class ParticleTransformerTaggerElectron(nn.Module):
    def __init__(
        self,
        pf_input_dim,
        sv_input_dim,
        ...
        **kwargs,
        ...
        self.rh_trimmer = SequenceTrimmer(enabled=trim and not for_inference)
        ...
        self.rh_embed = Embed(rh_input_dim, embed_dims, activation=activation)
    ...
    def forward(self,
                ...
                rh_x=None, rh_v=None, rh_mask=None,
        ...
        with torch.no_grad():
            ...
            rh_x, rh_v, rh_mask, _ = self.rh_trimmer(rh_x, rh_v, rh_mask)
            # lepton_x, lepton_v, lepton_mask, _ = self.lepton_trimmer(lepton_x, lepton_v, lepton_mask)
            v = torch.cat([pf_v, sv_v, lt_v, rh_v, lepton_v], dim=2)
            mask = torch.cat([pf_mask, sv_mask, lt_mask, rh_mask, lepton_mask], dim=2)

        pf_x = self.pf_embed(pf_x)  # after embed: (batch, seq_len, embed_dim)
        sv_x = self.sv_embed(sv_x)
        lt_x = self.lt_embed(lt_x)
        rh_x = self.rh_embed(rh_x)
        lepton_x = self.lepton_embed(lepton_x)
        x = torch.cat([pf_x, sv_x, lt_x, rh_x, lepton_x], dim=1)

        return self.part(x, v, mask)

class ParticleTransformerTaggerMuon(nn.Module):
    def __init__(
        self,
        pf_input_dim,
        lt_input_dim,
    ...
    def forward(self,
        ...
        sv_x = self.sv_embed(sv_x)
        lepton_x = self.lepton_embed(lepton_x)
        x = torch.cat([pf_x, lt_x, sv_x, lepton_x], dim=1)
