...
class ParticleTransformerTaggerForFinetune(nn.Module):
    def __init__(self, finetune_kw=dict(), **kwargs) -> None:
        ...
        print ('for_inference:', self.for_inference)
    ...
    def forward(self, *args):
        ...
        if self.for_inference:
            print ('adding softmax / output before softmax:', output)
            if self.mode == 'cls':
...
def get_save_fn(data_config, **kwargs):
    print ('get_save_fn kwargs:', kwargs)
    finetune_kw = kwargs.get('finetune_kw', None)
    if finetune_kw is None:
        return save_hybrid
    else:
...
def save_hybrid(args, data_config, scores, labels, observers):
    ...
    print ('label_cls_nodes:', label_cls_nodes)
    print ('split_reg:', split_reg)
    print ('composed_split_reg:', composed_split_reg)
    print ('label_stored:', label_stored)
...
def save_guass_regression(args, data_config, scores, labels, observers):
    print ('save_guass_regression args:', args)
