...
class GlobalParticleTransformerExporter(nn.Module):
    ...
    def forward(self, *args):
        ...
        # output = torch.cat([output, output_/aux], dim=1)
        # output = torch.cat([output], dim=1)

        # print lengths of output, output_cls, output_rest, output_aux
        print ("Output shape:", output.shape)
        print ("Output cls shape:", output_cls.shape)
        print ("Output rest shape:", output_rest.shape)
    ...
    def postprocess(self, output):
        ...
        if self.glopart_version in ['beta4p1', 'beta4p1:selected_indices']:
            ...
            output_nodes_expr = [
                ...
                'massCorrResonance', # massCorrResonance #my addition here
...
def get_model(data_config, **kwargs):
    print ("in get_model, kwargs:", kwargs)
    ...
    cfg = dict(
        ...
        # pair_embed_dims=kwargs.get('pair_embed_dims', (64, 64, 64)),
        num_heads=8,
        ...
        trim=True,
        for_inference=False,
        # for_inference=kwargs.get('for_inference')
    )
    ...
    cfg.update(
        ...
        return_embed=False, # return the last embed layer before FC

    )
    print ("Model config after update with kwargs:", cfg)
    print (finetune_kw)

    if finetune_kw is None:
        # print ("Initializing model in normal training mode")
        print ("Initializing model in normal training mode ?!?!?!?")
