...
class GlobalParticleTransformerExporter(nn.Module):
    ...
    def forward(self, *args):
        # process main model
        print (f"Running forward pass of GlobalParticleTransformerExporter with version {self.glopart_version}")
        print (f"Input args: {[arg.shape for arg in args]}")
        ...
        # output_aux = output[:, self.target_inds]
        ...
        # print the length of the output_cls and output_rest
        print (f"output_cls shape: {output_cls.shape}, output_rest shape: {output_rest.shape}")
        output_cls = torch.softmax(output_cls, dim=1)
        # check that the sum of output_cls is 1 along dim=1
        print (f"Sum of output_cls along dim=1 (should be 1): {torch.sum(output_cls, dim=1)}")
    ...
    def postprocess(self, x, output):
        ...
        if self.glopart_version in ['beta4p1', 'beta4p1:selected_indices']:
            ...
            print ("Post-processing output of GlobalParticleTransformerExporter for version %s" % self.glopart_version)
            ...
            # print lengnth of class_labels and aux_class_labels
            print ("length class_labels:", len(class_labels), "length of aux_class_labels:", len(aux_class_labels))
            ...
            assert len(output_varname) == 374+2+374 # beta4 setup
            
            if self.glopart_version == 'beta4p1:selected_indices':
                print ("Selected indices for output nodes:", self.glopart_selected_indices)
                ...
                # output_varname = [output_varname[:750][i] for i in self.glopart_selected_indices]
                output_varname = [output_varname[i] for i in self.glopart_selected_indices]
            ...
            for name, expr in replace_map.items():
                print(f"  {name}: {expr}")


            print ("Output variable names and their corresponding indices in the output tensor:")
            for i, name in enumerate(output_varname):
                print(f"  {name}: {i}")
            ...
            for i, expr in enumerate(output_nodes_expr):
                ...
                print (f"Calculated output node {i} with expression: {expr}, shape: {output_nodes[-1].shape}")
            output_nodes = torch.stack(output_nodes, dim=1)

            print ("Output nodes expressions and their corresponding variable names:")
            for i, expr in enumerate(output_nodes_expr):
                print(f"  {i}: {expr}")

            # return torch.concat([output_nodes, x], dim=1) # append hidden layer x after output nodes
            return torch.concat([output_nodes], dim=1) # append hidden layer x after output nodes
