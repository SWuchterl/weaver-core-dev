        ...
        with torch.autocast('cuda', enabled=self.use_amp):
            ...
            if self.for_inference:
                ...
                if self.export_params.get('apply_softmax', True):
                    ...
                    output = torch.cat([new_probs, selected_mass_corr], dim=1)  # 15 + 4 = 19 feature outputs
            ...
            if not self.return_embed:
                print ("returning output with shape", output.shape)
                return output
            else:
                print ("returning output and x_cls with shapes", output.shape, x_cls.shape)
                return output, x_cls
...
class ParticleTransformerTagger_ncoll(nn.Module):
    ...
    def __init__(self,
        ...
        print ("setting up model", input_dims, self.num_colls)
    ...
    def forward(self, *args):
        ...
        print ("forward called with", len(args), "inputs")
        print ("args are", [a for a in args])
        ...
        with torch.autocast('cuda', enabled=self.use_amp):
            ...
            print ("concatenating", len(x), "inputs")
            x = torch.cat(x, dim=1)
