...
# modified code
# def pairwise_lv_fts(xi, xj, num_outputs=4, eps=1e-8, for_onnx=False):
#     # ensure numerical guards are active in the conversion
#     pti, rapi, phii = to_ptrapphim(xi, False, eps=eps, for_onnx=for_onnx).split((1, 1, 1), dim=1)
#     ptj, rapj, phij = to_ptrapphim(xj, False, eps=eps, for_onnx=for_onnx).split((1, 1, 1), dim=1)

#     # protect against identical 4-momenta (e.g. duplicated lepton present among PF candidates)
#     # identical 4-vectors can produce zero distances / divisions that lead to NaNs
#     # add a tiny deterministic jitter to xj where xi == xj within numerical tol
#     try:
#         tol = eps * 1e-3
#         identical = ((xi - xj).abs().max(dim=1, keepdim=True).values < (tol)).to(xi.dtype)
#         if identical.any():
#             xi = xi.clone()
#             xj = xj.clone()
#             xj = xj + identical.expand_as(xj) * tol
#             # recompute cached derived quantities for jittered entries
#             pti, rapi, phii = to_ptrapphim(xi, False, eps=eps, for_onnx=for_onnx).split((1, 1, 1), dim=1)
#             ptj, rapj, phij = to_ptrapphim(xj, False, eps=eps, for_onnx=for_onnx).split((1, 1, 1), dim=1)
#     except Exception:
#         # keep original behaviour if shape assumptions differ
#         pass
...
#     if num_outputs > 4:
#         # guard the invariant-mass computation for xi-xj as well
#         lnds2 = torch.log(torch.clamp(-to_m2(xi - xj, eps=eps), min=eps))
#         outputs.append(lnds2)
...
#     # final safety: replace any NaN/Inf that may remain from numerical edge cases
...
class ParticleTransformerTagger(nn.Module):
    ...
    def __init__(self,
        ...
        # print input dims
        # print('pf_input_dim: %d' % pf_input_dim)
        # print('lt_input_dim: %d' % lt_input_dim)
        # print('sv_input_dim: %d' % sv_input_dim)
        # print('lepton_input_dim: %d' % lepton_input_dim)

        self.pf_trimmer = SequenceTrimmer(enabled=trim and not for_inference)
        self.lt_trimmer = SequenceTrimmer(enabled=trim and not for_inference)
        self.sv_trimmer = SequenceTrimmer(enabled=trim and not for_inference)
        self.lepton_trimmer = SequenceTrimmer(enabled=trim and not for_inference)


        self.pf_embed = Embed(pf_input_dim, embed_dims, activation=activation)
        self.lt_embed = Embed(lt_input_dim, embed_dims, activation=activation)
        self.sv_embed = Embed(sv_input_dim, embed_dims, activation=activation)
        self.lepton_embed = Embed(lepton_input_dim, embed_dims, activation=activation)
    ...
    def forward(self,
                pf_x, pf_v=None, pf_mask=None,
                lt_x=None, lt_v=None, lt_mask=None,
                sv_x=None, sv_v=None, sv_mask=None,
                lepton_x=None, lepton_v=None, lepton_mask=None):
        # x: (N, C, P)
        # v: (N, 4, P) [px,py,pz,energy]
        # mask: (N, 1, P) -- real particle = 1, padded = 0
        ...
        with torch.no_grad():
            ...
            # lepton_x, lepton_v, lepton_mask, _ = self.lepton_trimmer(lepton_x, lepton_v, lepton_mask)
            v = torch.cat([pf_v, lt_v, sv_v, lepton_v], dim=2)
            mask = torch.cat([pf_mask, lt_mask, sv_mask, lepton_mask], dim=2)

        with torch.cuda.amp.autocast(enabled=self.use_amp):
            pf_x = self.pf_embed(pf_x)  # after embed: (seq_len, batch, embed_dim)
            lt_x = self.lt_embed(lt_x)
            sv_x = self.sv_embed(sv_x)
            lepton_x = self.lepton_embed(lepton_x)
            x = torch.cat([pf_x, lt_x, sv_x, lepton_x], dim=0)
